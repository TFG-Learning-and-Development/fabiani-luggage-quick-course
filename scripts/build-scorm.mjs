...
function inferBasePath(html) {
  const astroAsset = html.match(/(?:href|src)=(["'])(\/(?:[^"']*\/)?_astro\/)[^"']*\1/);
  if (!astroAsset) {
    return '/';
  }

  const markerIndex = astroAsset[2].lastIndexOf('/_astro/');
  return markerIndex > 0 ? `${astroAsset[2].slice(0, markerIndex)}/` : '/';
}

function rewriteRootRelativeUrls(html, prefix, basePath) {
  const rewrite = (url) => {
    const relativeUrl = basePath !== '/' && url.startsWith(basePath)
  ...
  return html
    .replace(/(href=)(["'])(\/(?!\/)[^"']*)/g, (_match, attribute, quote, url) => `${attribute}${quote}${rewrite(url)}`)
    .replace(/(src=)(["'])(\/(?!\/)[^"']*)/g, (_match, attribute, quote, url) => `${attribute}${quote}${rewrite(url)}`)
    .replace(/url\((["']?)(\/(?!\/)[^)"']*)/g, (_match, quote, url) => `url(${quote}${rewrite(url)}`);
}
